...
df = ps.DataFrame(images)
df.head()

# COMMAND ----------

Image.open(io.BytesIO(df.content.iloc[50])).convert("L")
...
NUM_EPOCHS = 10   # iterate over all images 5 times
...
# argmin = {'lr': 0.006394452481493362, 'momentum': 0.004122620344750792}
# argmin = {'lr': 0.0026295031532699857, 'momentum': 0.0009070773353583252}
...
def train_and_evaluate_hvd(lr=0.001, momentum=0.0):
  ...
  optimizer = torch.optim.Adam(model.classifier[1].parameters(), lr=lr * hvd.size())
  # optimizer = torch.optim.SGD(model.fc.parameters(), lr=lr * hvd.size(), momentum=momentum)
...
with mlflow.start_run(run_name="MobileNet_exp1_adam") as run:  
# with mlflow.start_run(run_name="ResNet18_exp1") as run:  
  ...
  mlflow.log_param("lr", argmin["lr"])
  mlflow.log_param("momentum", argmin["momentum"])
